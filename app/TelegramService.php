...
class TelegramService
{
    ...
    {
        $this->logger = $logger;
        $this->telegramClient = new TelegramClient($this->logger);
    }

    private function shouldUpdate(): bool
    {
        return $_ENV['TELEGRAM_UPDATE'] === 'true';
    }
    ...
    public function getPosts(): array
    {
        ...
        $context = Context::new($this->logger);

        $response = $this->telegramClient->getPosts($context->offset);

        $context->offset = $response->offset + 1;

        $context->posts = [
            ...$context->posts,
            ...$this->telegramClient->downloadPostImages($response->posts)
        ];
